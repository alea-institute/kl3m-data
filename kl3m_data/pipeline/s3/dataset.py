...
class DatasetDocument:
    ...
    def __init__(
        ...
        self.doc_id = path_parts[-1].split('.')[0] if len(path_parts) > 1 else document_key
        ...
        for stage in [S3Stage.DOCUMENTS.value, S3Stage.REPRESENTATIONS.value, S3Stage.PARQUET.value]:
        ...
        self.representation_key = f"{S3Stage.REPRESENTATIONS.value}/{dataset_id}/{self.doc_path}"
    ...
    def get_representation_data(self) -> Optional[Dict]:
        ...
        rep_bytes = get_object_bytes(self.s3_client, self.bucket, self.representation_key)
        if not rep_bytes:
            return None
    ...
    def process_to_parquet(self) -> bool:
        ...
        try:
            ...
            if not documents:
                LOGGER.warning(f"No documents found in representation data for '{self.doc_id}'")
                return False
...
class DatasetPipeline:
    ...
    def iter_stage_keys(self, stage: S3Stage) -> Generator[str, None, None]:
        ...
        for key in iter_prefix(self.s3_client, self.bucket, prefix):
            key_count += 1
            if key_count % 1000 == 0:
                LOGGER.info(f"Found {key_count} keys in {stage.value}/{self.dataset_id}")
            yield key
        
        # Log the final count
        LOGGER.info(f"Found {key_count} total keys in {stage.value}/{self.dataset_id}")

    def iter_documents(self, stage: S3Stage = S3Stage.DOCUMENTS) -> Generator[DatasetDocument, None, None]:
        """
    ...
    def get_missing_documents(self, source_stage: S3Stage, target_stage: S3Stage) -> Set[str]:
        """
        ...
        """
        LOGGER.info(f"Finding documents in '{source_stage.value}' missing from '{target_stage.value}'")
        ...
        if self.key_prefix:
            ...
            result = {f"{self.dataset_id}/{self.key_prefix.strip('/')}/{path}" for path in missing_paths}
        else:
            result = {f"{self.dataset_id}/{path}" for path in missing_paths}
        
        LOGGER.info(f"Found {len(result)} documents in '{source_stage.value}' missing from '{target_stage.value}'")
        return result
    ...
    def process_stage_with_clobber(
        ...
        Returns:
            Tuple[int, int]: (processed_count, error_count)
        """
        LOGGER.info(f"Processing ALL documents {source_stage.value} -> {target_stage.value} for dataset '{self.dataset_id}'")
        LOGGER.info(f"Parameters: max_workers={max_workers}, max_size={max_size}, clobber=True")
        ...
        LOGGER.info(f"Found {len(task_keys)} total documents to process with clobber=True")
        
        # Process the documents
        return self._process_documents(task_keys, process_fn, max_workers, start_time, total_count)
    ...
    def process_stage_missing_only(
        ...
        Returns:
            Tuple[int, int]: (processed_count, error_count)
        """
        LOGGER.info(f"Processing MISSING documents {source_stage.value} -> {target_stage.value} for dataset '{self.dataset_id}'")
        LOGGER.info(f"Parameters: max_workers={max_workers}, max_size={max_size}, clobber=False")
        ...
        LOGGER.info(f"Finding missing documents between {source_stage.value} and {target_stage.value}...")
    ...
    def _get_process_function(
        ...
        """
        if source_stage == S3Stage.DOCUMENTS and target_stage == S3Stage.REPRESENTATIONS:
            LOGGER.info(f"Processing documents to representations")
            return lambda doc: doc.process_to_representations(max_size)
        elif source_stage == S3Stage.REPRESENTATIONS and target_stage == S3Stage.PARQUET:
            LOGGER.info(f"Processing representations to parquet")
            return lambda doc: doc.process_to_parquet()
        else:
            LOGGER.error(f"Invalid stage transition requested: {source_stage} -> {target_stage}")
            raise ValueError(f"Invalid stage transition: {source_stage} -> {target_stage}")
    ...
    def _process_documents(
        ...
        # Process in parallel
        LOGGER.info(f"Starting parallel processing with {max_workers} workers for {len(task_keys)} documents")
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in future_to_key:
                ...
                if completed_count % 100 == 0 or completed_count == len(future_to_key):
                    ...
                    LOGGER.info(f"Progress: {completed_count}/{len(future_to_key)} documents ({progress_pct:.1f}%) - Success: {processed_count}, Errors: {error_count}")
        
        end_time = datetime.datetime.now()
        duration = (end_time - start_time).total_seconds()
        docs_per_second = processed_count / duration if duration > 0 else 0
        
        LOGGER.info(f"Processing complete in {duration:.2f} seconds ({docs_per_second:.2f} docs/sec)")
        LOGGER.info(f"Results: Processed: {processed_count}, Errors: {error_count}, Total scanned: {total_count}")
    ...
    def build_index(self) -> bool:
        ...
        try:
            ...
            result = put_object_bytes(self.s3_client, self.bucket, index_path, compressed_data)
            
            if result:
                LOGGER.info(f"Successfully built index with {len(all_keys)} objects at {index_path}")
            else:
